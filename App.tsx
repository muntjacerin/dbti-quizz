...
export default function App() {
  ...
  const handleAnswer = (mappedTypes: string[]) => {
    const newScores = { ...scores };

    // Balanced scoring:
    // Each answer is worth exactly 1 point in total.
    // If an option maps to two types, each gets 0.5 instead of both getting 1.
    // This prevents multi-type answers from accidentally overpowering single-type answers.
    const pointsPerType = mappedTypes.length > 0 ? 1 / mappedTypes.length : 0;

    mappedTypes.forEach(type => {
      if (newScores[type] !== undefined) {
        newScores[type] += pointsPerType;
      }
    });
  ...
  const calculateResult = (finalScores) => {
    // Sort types by score
    const sorted = Object.entries(finalScores).sort((a, b) => Number(b[1]) - Number(a[1]));
    const maxScore = Number(sorted[0][1]);

    // Tie handling:
    // The old version always picked the first type in DBTI_TYPES when scores tied.
    // This version randomly chooses among all top-scoring types.
    const tiedTopIds = sorted
      .filter(([, score]) => Number(score) === maxScore)
      .map(([id]) => id);

    const primaryId = tiedTopIds[Math.floor(Math.random() * tiedTopIds.length)];
    const primaryType = DBTI_TYPES.find(t => t.id === primaryId);

    const sortedForShifts = sorted.filter(([id]) => id !== primaryId);

    // Ensure we don't divide by zero
    const base = maxScore > 0 ? maxScore : 1;

    // Shifts (Next 3 likely types with percentages relative to max score)
    const shifts = sortedForShifts.slice(0, 3).map(item => {
      const typeInfo = DBTI_TYPES.find(t => t.id === item[0]);
      let percentage = Math.round((Number(item[1]) / base) * 100);
    ...
    // Helpful for testing: open browser Console to see the full score table.
    console.table(
      sorted.map(([id, score]) => ({
        id,
